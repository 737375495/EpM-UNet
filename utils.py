...
class myNormalize:
    def __init__(self, data_name, train=True):
        if data_name == 'FIO-EP':
            if train:
                # 使用你自己计算的训练集的均值和标准差
                self.mean = [0.29013144, 0.36241937, 0.43037119]  # 例如，RGB的均值
                self.std = [0.054931, 0.06351891, 0.05092916]   # 例如，RGB的标准差
            else:
                # 你可以设置不同的测试集均值和标准差
                self.mean = [0.29194426, 0.38935262, 0.47497574]  # 这里可以和训练集相同
                self.std = [0.04796632, 0.05459418, 0.04766337]   # 这里可以和训练集相同
